 ...
 * @returns {boolean} <code>true</code> if the header is valid, <code>false</code> otherwise.
 * @author Lukas Trommer
 */
...
Authorization.extractAccessToken = function (authorizationHeader) {
    if (authorizationHeader) {
        let matches = authorizationHeader.match(HTTP_HEADER_EXTRACT_REGEX);
 ...
 * @author Lukas Trommer
 */
Authorization.storeAuthorization = async function (user, accessToken, transaction = undefined) {
    let dbHandle = new UserDatabaseHandle();
    let hash = Authorization.hashAccessToken(accessToken);
...
Authorization.authorizeUser = async function (user, accessToken) {
    ...
    let dbHandle = new UserDatabaseHandle();
    let accessTokenHash = await dbHandle.queryUserAccess(user);

    if (accessTokenHash) {
        // Compare provided access token to actual access token hash and return
        return Authorization.compareAccessToken(accessToken, accessTokenHash.toString());
    } else {
